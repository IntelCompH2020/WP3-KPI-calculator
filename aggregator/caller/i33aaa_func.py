...
def ind_caller(enco, results, logging, extra_aggr_param=[], working_path=""):
    ...
    try:
        ...
        results["i33aaa"]["sv01"] = None
        print(f"Error calculating i33aaa[sv01]: {str(e)}")
    ...
    try:
        ...
        results["i33aaa"]["sv21"] = None
        print(f"Error calculating i33aaa[sv21]: {str(e)}")
    ...
    try:
        ...
        results["i33aaa"]["sv09"] = df_dict_new
    except Exception as e:
        results["i33aaa"]["sv09"] = None
        print(f"Error calculating i33aaa[sv09]: {str(e)}")
        
    try:
        # Convert DataFrame to dictionary
        df_dict = (
            df.groupby(["company_name", "CompanySize"])["total_trademarks"]
            .sum()
        ) / len(df)
        df_dict = df_dict.to_dict()
        df_dict_new = {}
        for k, v in df_dict.items():
            ...
            df_dict_new[company][country] = v
        # Post-processing to get desired format
        results["i33aaa"]["sv15"] = df_dict_new
    except Exception as e:
        results["i33aaa"]["sv15"] = None
        print(f"Error calculating i33aaa[sv15]: {str(e)}")
