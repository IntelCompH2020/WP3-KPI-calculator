...
def ind_caller(pat, results, extra_aggr_param=[]):
    ...
    numerator = uf.secondary_view(pat, "topic", i13_aggregation_npl, extra_aggr_param)
    ...
    denominator = uf.secondary_view(pat, "category", i13_aggregation, extra_aggr_param)
    results["i13"]["sv03"] = {}
    for k in numerator.keys():
        if denominator[k] == 0:
            denominator[k] = 1
    ...
    numerator = uf.inner_secondary_view(pat, "participant.name", i13_aggregation_npl)
    ...
    numerator = uf.inner_secondary_view(pat, "participant.sector", i13_aggregation_npl)
    denominator = uf.inner_secondary_view(pat, "participant.sector", i13_aggregation)
    results["i13"]["sv08"] = {}
    for k in numerator.keys():
        if denominator[k] == 0:
            denominator[k] = 1
    ...
    numerator = uf.inner_secondary_view(pat, "participant.country", i13_aggregation_npl)
    denominator = uf.inner_secondary_view(pat, "participant.country", i13_aggregation)
    full_set = {}
    for k in numerator.keys():
        if denominator[k] == 0:
            denominator[k] = 1
