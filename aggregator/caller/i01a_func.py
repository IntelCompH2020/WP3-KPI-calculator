...
def i01_aggregation_per_year(field, extra_aggr_param):
    return extra_aggr_param + [
...
def ind_caller(sci, results, logging, extra_aggr_param=[], working_path=""):
    ...
    try:
        full_set = uf.inner_secondary_view_per_year(
            sci, "affiliations.country", i01_aggregation_per_year, extra_aggr_param, first_year=2000
        )
        results["i01"]["sv09"] = {}
        for k in full_set.keys():
            results["i01"]["sv09"][k] = full_set[k]
    except Exception as e:
        results["i01"]["sv09"] = None
        print(f"Error calculating i01[sv09]: {str(e)}")
        logging.error(f"Error calculating i01[sv09]: {str(e)}")
